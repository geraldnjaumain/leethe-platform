# Leethe Developer Platform `v1.0.0-alpha`
...
* **100% Native Go Backend**: All 4 bounded context services (`identity`, `vcs-engine`, `compute-engine`, `edge-proxy`) and the developer CLI (`apps/cli`) are implemented in native **Go (Golang)**.
...
* **Instant Zero-Downtime Rollbacks (<10ms)**: Atomic target switching via Pingora/Caddy dynamic proxies allows 1-click rollbacks in **0.0060ms** without dropping active connections.
...
│
├── services/                  # Native Go Bounded Context Services
│   ├── identity/              # Go Passkey Auth, Session Tokens, FGAC Policy Evaluator (:8081)
│   ├── vcs-engine/            # Go Smart HTTP/SSH Server, Repo Storage, Diff Parser (:8082)
│   ├── compute-engine/        # Go Nixpacks Auto-Builder, Container Runner, Log Streamer (:8083)
│   └── edge-proxy/            # Go Dynamic Proxy Router & <10ms Rollback Engine (:8084)
│
├── infrastructure/            # Multi-Container Production Stack
│   ├── docker/                # Docker Compose (Identity, VCS, Compute, Proxy, Postgres, Redis)
│   └── nginx/                 # Caddyfile Dynamic Proxy Configuration
│
├── tests/
│   └── e2e/                   # End-to-End Go Integration Test Suite
│
└── packages/
...
## Developer Quickstart & Verification Harness
...
* **Rollback Controller**: 1-click deployment target switching (0.0060ms latency).

### 2. Run End-to-End Go Services Test Suite
```bash
make e2e
```

### 3. Run CLI Command Prototype
```bash
make build-cli
# or: go run apps/cli/main.go --help
```

### 4. Production Self-Hosting with Docker Compose
...
* [`AGENTS.md`](file:///Users/tera/Documents/leethe/.agents/AGENTS.md) — Adversarial Code Reviewer Protocol, Visual Verification Standards, Anti-Hallucination Rules, and Mandatory Go Backend Policy.
* [`SCOPE.md`](file:///Users/tera/Documents/leethe/SCOPE.md) — Feature boundaries, 14 phase completion gates, and non-goals.
